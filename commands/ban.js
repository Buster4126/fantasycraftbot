...
module.exports.run = async (client, message, argument) => {
    ...
    if (!message.member.hasPermission("BAN_MEMBERS")) return message.reply("Sorry, you cannot do this");

    if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.reply("You don't have the perms");

    var banUser = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[1]));

    var reason = args.slice(2).join(" ");

    if (!banUser) return message.reply("Cannot find that user!");
    ...
    message.channel.send(embedPrompt).then(async msg => {
        ...
        // message.channel.awaitMessages(m => m.author.id == author.id,
        ...
        //      }).catch(() => {
        //          message.reply('You didn't react within 30 seconds, canceled');
        //      });
...
async function promptMessage(message, author, time, reactions) {
    ...
    // Dan kunnen we bericht terug sturen met dat icontje dat is aangeduid.
